...
- **`demo`** (default) — deterministic, dependency-free provider. Reports
  ...
  statistics, and — crucially — the `/api/simulate` endpoint (used by the
  mobile app's Demo Mode buttons) always returns a realistic, high-confidence
  detection for the requested scenario. This guarantees the full workflow
...
- **`gemini`** — sends the frame to Google's Gemini vision model
  ...
  than crashing frame ingestion. `/api/simulate` (Demo Mode) never calls
  Gemini — it reuses the same guaranteed templates as `demo`/`real`, so a
  live demo never depends on third-party API latency or availability.
  ...
  continuous live monitoring against this provider instead of Demo Mode.
